...
class Logger:
    ...
    def save_state(self, state_dict, itr=None):
        ...
        will save whatever is in ``state_dict``---usually just a copy of the
        environment---and the most recent parameters for the model you 
        previously set up saving for with ``setup_tf_saver`` or ``setup_pytorch_saver``. 
        ...
        Args:
            ...
            itr: An int, a string, or None. Current iteration of training.
                Can be 'best' to save the best model.
        """
        if proc_id()==0:
            if itr is None:
                fname = 'vars.pkl'
            elif isinstance(itr, str):
                fname = 'vars%s.pkl' % itr  # 支持字符串，如 'best'
            else:
                fname = 'vars%d.pkl' % itr  # 整数
    ...
    def _pytorch_simple_save(self, itr=None):
        ...
        if proc_id()==0:
            ...
            fpath = 'pyt_save'
            fpath = osp.join(self.output_dir, fpath)
            fname = 'model' + ('%s'%itr if itr is not None else '') + '.pt'
            fname = osp.join(fpath, fname)
            os.makedirs(fpath, exist_ok=True)
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
